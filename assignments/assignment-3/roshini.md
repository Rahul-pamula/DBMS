# Assignment 3: SQL Constraints & Referential Integrity

**SQL Constraints** are rules enforced on data columns in a database table to ensure the accuracy, reliability, and integrity of the data stored within the database. Constraints can be column-level or table-level and prevent invalid data entry or illegal relationships between tables.

---

## Constraints Assignment Questions & SQL Solutions
...
*Explanation:* 
When a parent record in `doctors` is referenced by child records in `appointments` without `ON DELETE CASCADE` enabled (i.e. using `RESTRICT` or `NO ACTION`), the DBMS blocks the operation to preserve referential integrity.
...
## Conclusion
SQL constraints (`NOT NULL`, `UNIQUE`, `DEFAULT`, `CHECK`, `PRIMARY KEY`, `FOREIGN KEY`, and Composite Keys) enforce data integrity, business rules, and referential relationships across tables. Understanding cascade options like `ON DELETE CASCADE` prevents orphan records while maintaining consistent relational databases.
